...
fingerprint_dict = model.load_pickle('./test/example/fingerprint_dict.pickle')
atom_dict = model.load_pickle('./test/example/atom_dict.pickle')
bond_dict = model.load_pickle('./test/example/bond_dict.pickle')
edge_dict = model.load_pickle('./test/example/edge_dict.pickle')
word_dict = model.load_pickle('./test/example/sequence_dict.pickle')
...
def main() :
    ...
    kr_model = model.enzykr(device, 801, 916, dim, layer_gnn, window, layer_cnn, layer_output).to(device)
    #kr_model = model.enzykr().to(device)
    kr_model.load_state_dict(torch.load('./output/train_denov_224', map_location=device))
    #print(state_dict.keys())
    # model.eval()
    predictor = Predictor(kr_model)
    ...
    with open('./test/output.tsv', 'w') as outfile :
